...
def getVideoStreams(video_path):
    ...
    while cap.isOpened():
        ...
        skip_count = 0
        # cv2.imwrite('temp.jpg', frame)
        frame,head_x,head_y,left_hand_coordinates,right_hand_coordinates,elbow_angle,knee_angle,elbow_coo,knee_coo, left_shoulder = get_angles_postions(frame)
        img,boxes,scores,classes, height, width = detect_API(frame, 'temp.jpg',[])
        print("HEIGHTS:",(height, width))
        if 0 not in classes[0]:
        ...
        output_video.write(frame)
    trajectory_fit(shot_tracking,height,width,'/Users/igautam/Documents/GitHub/basketball-shot-analysis/output/traces/')
    os.remove("temp.jpg")
    delete_folder_contents(runs_folder)
...
if __name__ == '__main__':
    getVideoStreams('../input/two_score_three_miss.mp4')