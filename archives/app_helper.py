...
def getBallAndRim(sess,img):

    height, width = img.shape[:2]
    detection_graph, image_tensor, boxes, scores, classes, num_detections = tensorflow_init()
    img_expanded = np.expand_dims(img, axis=0)
    (boxes, scores, classes, num_detections) = sess.run([boxes, scores, classes, num_detections],feed_dict={image_tensor: img_expanded})
    valid_detections = 0
...
'''
def getBallAndRim(img):
    cv2.imwrite('temp.jpg', img)
    img,boxes,scores,classes,num_detections = run(weights='./weights/basket_rim.pt', source='temp.jpg', conf_thres=0.1, project='runs', name='run')
    return img,boxes,scores,classes,num_detections
'''
